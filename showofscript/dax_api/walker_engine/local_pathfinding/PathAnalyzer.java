...
public class PathAnalyzer {
	...
	public static RealTimeCollisionTile closestTileInPathToPlayer(List<RSTile> path) {
		...
		closestToPlayer = (RealTimeCollisionTile) BFS.bfsClosestToPath(path,
				RealTimeCollisionTile.get(playerPosition.getX(), playerPosition.getY(), playerPosition.getPlane()));
	...
	public static DestinationDetails furthestReachableTile(List<RSTile> path, PathFindingNode currentPosition) {
		...
		outside: for (int i = path.indexOf(currentPosition.getRSTile()); i < path.size() && i >= 0; i++) {
			RSTile currentNode = path.get(i);
			RealTimeCollisionTile current = RealTimeCollisionTile.get(currentNode.getX(), currentNode.getY(),
					currentNode.getPlane());
			if (current == null) {
				return null;
			}
			if (i + 1 >= path.size()) {
			...
			RealTimeCollisionTile next = RealTimeCollisionTile.get(nextNode.getX(), nextNode.getY(),
					nextNode.getPlane());
			Direction direction = directionTo(current.getRSTile(), nextNode);
			if (direction == Direction.UNKNOWN) {
				furthestReachable = current;
				return new DestinationDetails(PathState.DISCONNECTED_PATH, current, nextNode.getX(), nextNode.getY(),
						nextNode.getPlane());
			}
			if (!direction
					.confirmTileMovable(RealTimeCollisionTile.get(current.getX(), current.getY(), current.getZ()))) {

				for (int j = 1; j < 5 && j + i < path.size(); j++) {
					RSTile nextInPath = path.get(i + j);
					RealTimeCollisionTile nextInPathCollision = RealTimeCollisionTile.get(nextInPath.getX(),
							nextInPath.getY(), nextInPath.getPlane());
				...
				return new DestinationDetails(PathState.OBJECT_BLOCKING, current, nextNode.getX(), nextNode.getY(),
						nextNode.getPlane());
			}
			if (!Projection.isInMinimap(
					Projection.tileToMinimap(new RSTile(nextNode.getX(), nextNode.getY(), nextNode.getPlane())))) {
				furthestReachable = current;
				if (next != null) {
					return new DestinationDetails(PathState.FURTHEST_CLICKABLE_TILE, current, next);
				}
				return new DestinationDetails(PathState.FURTHEST_CLICKABLE_TILE, current, nextNode.getX(),
						nextNode.getY(), nextNode.getPlane());
			}
		}
		return null;
	}
	...
	public static class DestinationDetails {
		...
		private final RealTimeCollisionTile destination;
		private RealTimeCollisionTile nextTile;
		private final int assumedX;
		private final int assumedY;
		private final int assumedZ;

		public DestinationDetails(PathState state, RealTimeCollisionTile destination) {
			this.state = state;
	...
	public enum PathState {
		FURTHEST_CLICKABLE_TILE, DISCONNECTED_PATH, OBJECT_BLOCKING, END_OF_PATH
	}

	private enum Direction {
		NORTH(0, 1), EAST(1, 0), SOUTH(0, -1), WEST(-1, 0), NORTH_EAST(1, 1), SOUTH_EAST(1, -1), NORTH_WEST(-1, 1),
		SOUTH_WEST(-1, -1), SAME_TILE(0, 0), UNKNOWN(104, 104);
		...
		boolean confirmTileMovable(RealTimeCollisionTile realTimeCollisionTile) {
			...
			RealTimeCollisionTile destination = RealTimeCollisionTile.get(realTimeCollisionTile.getX() + this.x,
					realTimeCollisionTile.getY() + this.y, realTimeCollisionTile.getZ());
			if (destination == null) {
				return false;
			}
