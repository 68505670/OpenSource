...
public class BankEventV2 extends BotEvent {
    ...
    public void updateCache() {
        if (!Banking.isBankScreenOpen() && Banking.isBankLoaded()) {
            ...
            for(int i = 0; inventCache.length > i;i++) {
                int qty = 1;
                if(inventCache[i].getDefinition().isStackable()) {
                    qty = inventCache[i].getStack();
                } else {
                    qty = InventoryEvent.getItemCount(inventCache[i].getDefinition().getName());
                }
                General.println("Updating Inventory Cache: "+inventCache[i].getDefinition().getName()+" ID: "+inventCache[i].getDefinition().getID() + " qty: "+ qty);
                updateItem(inventCache[i].getDefinition().getName(), inventCache[i].getDefinition().getID(), qty);
            }
